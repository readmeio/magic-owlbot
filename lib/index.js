...
module.exports = {
  ...
  start: function() {
    ...
    if(!opts.slack.token) return console.error("[ERROR] You need to set OWLBOT_SLACK");
    if(!opts.twilio.phoneNumber) return console.error("[ERROR] You need to set OWLBOT_TWILIO_NUMBER");
    if(!opts.twilio.accountSid) return console.error("[ERROR] You need to set OWLBOT_TWILIO_ACCOUNTSID");
    if(!opts.twilio.authToken) return console.error("[ERROR] You need to set OWLBOT_TWILIO_AUTHTOKEN");
    if(!opts.twilio.magic) return console.error("[ERROR] You need to set OWLBOT_MAGIC");
  ...
  handleSlackQuery: function(query, channel) {
    ...
    if(query.length > 160) {
      var query_split = query.match(/.{1,10}/g);
      _.each(query_split, function(text, i) {
        setTimeout(function() {
          send("[" + i + "/" + query_split.length + "] " + text);
        }, 300 * i); // Try to space them out evenly
      });
    } else {
      send(query);
    }
